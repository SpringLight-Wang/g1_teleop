...
* **双模式支持**：
    * `g1_dual_real.py`: 实机控制模式 (基于 Unitree SDK2)。
...
| **左手柄姿态 (IMU)** | **左手腕部/肘部/小臂旋转** | 手柄的IMU直接映射到机械臂 (Pitch/Roll/Yaw) |
| **右手柄姿态 (IMU)** | **右手腕部/肘部/小臂旋转** | 同上 |
| **ZL 键 (左臂)** | **左臂模式切换** | 切换 **[🔒锁定/归位]** 与 **[🔓手动控制]** |
| **ZR 键 (右臂)** | **右臂模式切换** | 切换 **[🔒锁定/归位]** 与 **[🔓手动控制]** |
| **Ctrl + C** | **安全退出** | 脚本会尝试让手臂缓慢归位并释放控制权 |

### 2. 实机控制

该模式下，脚本控制上半身双机械臂，不影响官方机器人底层运控算法维持平衡，⚠️请务必使用吊架保护机器人，并确保周围有足够的安全缓冲空间⚠️。
...
1. **模型文件不存在？**
* 请检查是否修改MODEL_PATH，将路径改为你电脑中的实际路径。


2. **手柄连接报错？**
* 请检查是否安装了 `libhidapi-dev`。
* 尝试在系统蓝牙设置中删除设备并重新配对。
* 确保有读取 `/dev/hidraw*` 的权限。
...
* 本项目属于实验性质代码。
* **Sim2Real 存在风险**：请务必使用吊架保护机器人，并确保周围有足够的安全缓冲空间。
